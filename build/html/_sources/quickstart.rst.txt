...
Analysis
---------------------
* There are a few straightforward ways to analyze your connectivity network data. From the analyze menu, choose 'Stats -> Network Related -> Network Statistics Histograms'. Here, you can generate a number of histograms that describe your network:

.. image:: _static/network_histos.png
   :width: 400px
   :alt: histogram menu

* Regarding these histograms - some of them may be slow to compute for large networks. Some of them may also be skewed if you have multiple disconnected components in your graph (any ones that this can happen to will mention they are using multiple components in their title). To remove small, isolated components from your graph, you can right click your largest component in the network graph view in the bottom right and choose 'isolate component'.
* Each one of these histograms can be compared between datasets to find observable differences. They can also be used to threshold the nodes to find objects of interest. Right Click the table generated by the histogram in the top right and choose 'Use to Threshold Nodes'. Here, I am looking for objects with a high betweenness centrality in my kidney-glomeruli neural network:

.. image:: _static/threshold_nodes.png
   :width: 800px
   :alt: threshold_nodes

* If you don't know what histogram to create, you can use 'compute all analysis and export to CSV' to look at all the histograms together.

* Another way to do analysis is by grouping your nodes into communities. For a network, these might represent functional groups. use 'Analyze -> Create Communities Based on Network' to assign communities.
